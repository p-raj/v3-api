...
class ResourceSchema(models.Model):
    """
    Resource schema is actually the swagger client of any resource/service which admin will upload here.
    It gives us the insight of Resource's API's , methods it support etc.
    """
    # Relations
    resource = models.ForeignKey(Resource,
                 related_name="resource_schema",
                 verbose_name=_("resource"),
                 help_text=_('Resource to which this schema belongs too.'),
    )
    ...
    def __str__(self):
        return _(
                "Resource {0}".format(self.resource.name)
            )

    # Meta
    class Meta:
        verbose_name = _("Resource Schema")
        verbose_name_plural = _("Resources Schema")
        ordering = ["-created_at"]
